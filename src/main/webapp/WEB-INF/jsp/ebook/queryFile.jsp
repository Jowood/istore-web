...
<h1><a href="checkUserLogin.login">返回首页</a> > ${requestScope.projectName} > ${requestScope.name } </h1>
<form name="selectForm" method="post" action="queryFile.ebook?catalogKey=${requestScope.catalogKey}&name=${requestScope.name }" id="selectForm">
	<fieldset>
		<div>
...
<table id="users" class="ui-widget ui-widget-content">
		...
		<tbody> 
			...
			<td><a href="viewFile.ebook?projectName=${requestScope.projectName }&parentKey=${requestScope.catalogKey }&parentName=${requestScope.name }&catalogKey=${item.key}&name=${item.name}">${item.name}</a></td> 
			<td>&nbsp;</td>
			<td>&nbsp;</td>
			<td>&nbsp;
			</td>
